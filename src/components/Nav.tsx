...
export default function Nav({ currentPage, onNavigate }: NavProps) {
  ...
  const links: { label: string; page: 'home' | 'services' | 'contact' }[] = [
  ...
  return (
    <nav
      ...
      style={scrolled ? { backgroundColor: 'rgba(10, 22, 40, 0.88)' } : undefined}
    >
      <div className="max-w-6xl mx-auto px-6 py-4">
        ...
        <div
          className={`md:hidden overflow-hidden transition-all duration-300 ${
            menuOpen ? 'max-h-56 opacity-100 mt-4' : 'max-h-0 opacity-0'
          }`}
        >
          <div className="flex flex-col gap-1 pb-4 border-t pt-4" style={{ borderColor: 'rgba(34,211,238,0.1)' }}>
            {links.map(({ label, page }) => (
              <button
                key={page}
                onClick={() => onNavigate(page)}
